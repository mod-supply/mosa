...
const PROJECTS_DATA = {
  programming: [
    { title_ar:'مشروع برمجة و تطوير متجر الامداد العصري', title_en:'Modern Supply E-commerce Development Project', desc_ar:'تطوير الواجهات و برمجتها', desc_en:'Frontend Development & Implementation', image:'', link:'https://modernsupply.com.sa/' },
  ],
  branding: [
    { title_ar:'مشروع هوية العاصمة المميز للذهب', title_en:'Branding Project 01', desc_ar:'شعار واجهة ودليل هوية بصرية كاملة.', desc_en:'Full visual identity: logo, palette, brand guide.', image:'', link:'' },
...
function initScrollReveal() {
  const obs = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      /* امسح أي حالة قديمة */
      el.classList.remove('glitch-in', 'done');
      el.style.opacity   = '';
      el.style.transform = '';

      /* فريم واحد تأخير حتى يطبق الـ reset */
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          el.classList.add('glitch-in');
          if (el.id === 'skills') animateSkillBars();
        });
      });

      /* بعد انتهاء الأنيميشن اقفل الحالة */
...
/* ============================================================
   CUSTOM CURSOR — NEON RING + PARTICLES
   ============================================================ */
(function initCursor() {

  /* لا تشتغل على موبايل/تاتش */
  if (window.matchMedia('(hover: none) and (pointer: coarse)').matches) return;

  const ring = document.getElementById('cursor-ring');
  if (!ring) return;

  /* ── حركة الرينج بـ lerp ناعمة ── */
  let mx = -100, my = -100;
  let rx = -100, ry = -100;
  ...
  /* ── Hover على عناصر تفاعلية ── */
  const HOVER_EL = 'a, button, [role="button"], input, textarea, .proj-cat-card, .srv-card, .skill-item, label';

  document.addEventListener('mouseover', e => {
    if (e.target.closest(HOVER_EL))
      document.body.classList.add('cur-hover');
  }, { passive: true });

  document.addEventListener('mouseout', e => {
    if (!e.relatedTarget || !e.relatedTarget.closest?.(HOVER_EL))
  ...
  document.addEventListener('mousedown', e => {
    ...
    /* 10 جسيمات تتطاير بزوايا موزعة */
    const COLORS = ['var(--green)', 'var(--cyan)', 'var(--magenta)'];
    for (let i = 0; i < 10; i++) {
...
/* ============================================================
   KEYBOARD NAVIGATION — Arrow Up / Down بين الأقسام
   ============================================================ */
(function initKeyboardNav() {
  ...
  function goToSection(index) {
    const id = SECTIONS[Math.max(0, Math.min(index, SECTIONS.length - 1))];
    const el = document.getElementById(id);
    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  /* مؤشر بصري اختياري — يظهر اسم القسم لثانيتين */
  let indicator = null;
  let hideTimer = null;
  ...
  function showIndicator(index) {
    if (!indicator) {
      ...
      indicator.style.cssText = `
        ...
        border-radius: 4px;
        box-shadow: 0 0 12px var(--green), 0 0 24px #00ff9933;
        pointer-events: none;
        z-index: 500;
        opacity: 0;
    ...
    const label = document.querySelector(`#${id} .section-title`)?.textContent
                || (id === 'hero' ? '// home' : `// ${id}`);
  ...
  document.addEventListener('keydown', e => {
    ...
    /* تجاهل لو المستخدم يكتب في input أو textarea */
    const tag = document.activeElement?.tagName;
    if (tag === 'INPUT' || tag === 'TEXTAREA') return;

    if (e.key === 'ArrowDown') {
      e.preventDefault();
      const next = Math.min(getCurrentIndex() + 1, SECTIONS.length - 1);
      goToSection(next);
      showIndicator(next);
    }
